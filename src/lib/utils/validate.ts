/** 정규식 REGEX */
export const EMAIL_PATTERN_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; // EMAIL 패턴
export const ONLY_NUMBER_10_RE = /^\d{10}$/;                  // 숫자만 10자리
export const ONLY_NUMBER_8_RE = /^\d{8}$/;                    // 숫자만 8자리
export const PHONE_PATTERN_RE = /^010\d{8}$/;                 // 010 + 8자리 = 총 11자리
...
export const isValidPassword = (v: string) =>
  ...
  /[A-Za-z]/.test(v) && /\d/.test(v) && /[^A-Za-z0-9]/.test(v);
 ...
 */
export const isValidBirth = (s?: string | null) =>
  !!s && ONLY_NUMBER_8_RE.test(s) && isValidDateYYYYMMDD(s);
