...
export type LoginRes = {
    accessToken: string;
    refreshToken: string;
    accessTokenExpiresIn: number; // AT 만료: 30초
    refreshTokenExpiresIn: number; // RT 만료: 5분
};
...
export async function verifyBusinessNumber(businessNumber: string) {
    ...
    return res.data; // { company, owner, businessNumberVerifyToken }
}

// 회원가입
export async function registerUser(data: FormData) {
    const res = await api.post('/api/auth/register', data);
    return res.data; // { success: true, message: "회원가입이 완료되었습니다" }
}
